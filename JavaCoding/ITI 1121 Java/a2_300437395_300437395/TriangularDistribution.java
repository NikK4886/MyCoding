...
public class TriangularDistribution {
	...
	public TriangularDistribution(int a, int c, int b) {
		...
		if((a > c) || (c>b)){
			throw new ArithmeticException("init vars do not follow the parameters");
		} 
	...
	public Rational pdf(int x) {
		...
		if(x<a){
			chance = new Rational(0,1);
		}
		else if((a <= x) && (x<c)){
			chance = new Rational((2*(x-a)),((b-a)*(c-a)));
		}
		else if(x == c){
			chance = new Rational(2,(b-a));
		}
		else if((c < x) && (x <= b)){
			chance = new Rational((2*(b-x)),((b-a)*(b-c)));
		}
		else{
			chance = new Rational(0,1);
		}
