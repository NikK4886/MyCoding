...
public class Simulator {
	...
	public void simulate() {
		...
		boolean chanceOfCar, probaOfLeave;
		Spot spotEnter, spotInLot;
		Car carEnter, frontCar;
		int numRows, numSpotsPerRow, timeInLot, timeEnteredLot;
		Rational chanceOfLeave;
		CarType frontCarType;
		String frontObjStr, queueString,frontCarStr,frontPlate;
				
		
		numRows = lot.getNumRows();
		numSpotsPerRow = lot.getNumSpotsPerRow();
		...
		while (clock < steps) {
			...
			chanceOfCar = RandomGenerator.eventOccurred(probabilityOfArrivalPerSec);
			if(chanceOfCar){
				carEnter =  RandomGenerator.generateRandomCar();
				spotEnter = new Spot(carEnter, clock);
				incomingQueue.enqueue(spotEnter);
			}
			
			
			for(int i = 0; i < numRows; i++){
				for(int j = 0; j<numSpotsPerRow;j++){
					spotInLot = lot.getSpotAt(i,j);
					if(spotInLot == null){
						continue;
					}
					else{
						timeEnteredLot = spotInLot.getTimestamp();
						timeInLot = clock - timeEnteredLot;
						
						if(timeInLot == MAX_PARKING_DURATION){
							outgoingQueue.enqueue(spotInLot);
							spotInLot = this.lot.remove(i,j);
							
						}
						else{
							chanceOfLeave = departurePDF.pdf(timeInLot);
							probaOfLeave = RandomGenerator.eventOccurred(chanceOfLeave);
							if(probaOfLeave){
								outgoingQueue.enqueue(spotInLot);
								spotInLot = this.lot.remove(i,j);
							}
			...
			if(incomingQueue.isEmpty() != true){
				queueString = incomingQueue.toString();
				frontObjStr = queueString.substring(1,queueString.indexOf(","));
				frontCarStr = frontObjStr.substring(0,frontObjStr.indexOf("("));
				frontPlate = frontObjStr.substring(frontObjStr.indexOf("(")+1,frontObjStr.indexOf(")"));
				frontCarType = Util.getCarTypeByLabel(frontCarStr);
				frontCar = new Car(frontCarType, frontPlate);
				
				if(lot.attemptParking(frontCar,clock) == true){
					incomingQueue.dequeue();
					System.out.println(frontCar + " ENTERED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
				}
				
			}
			
			if(outgoingQueue.isEmpty() != true){
				queueString = outgoingQueue.toString();
				frontObjStr = queueString.substring(1,queueString.indexOf(","));
				frontCarStr = frontObjStr.substring(0,frontObjStr.indexOf("("));
				frontPlate = frontObjStr.substring(frontObjStr.indexOf("(")+1,frontObjStr.indexOf(")"));
				frontCarType = Util.getCarTypeByLabel(frontCarStr);
				...
				System.out.println(frontCar + " EXITED at timestep " + clock + "; occupancy is at " +lot.getTotalOccupancy());
			}
