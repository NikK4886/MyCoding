	...
	public static int getOptimalNumberOfSpots(int hourlyRate) {
		...
		while(ctrl){
			...
			for(int i = 0; i < NUM_RUNS; i++){
				...
				sim = new Simulator(lot,hourlyRate, SIMULATION_DURATION);
				long start = System.currentTimeMillis();
				sim.simulate();
				long end = System.currentTimeMillis();
				temp = sim.getIncomingQueueSize();
				queueSizes = queueSizes + sim.getIncomingQueueSize();
				System.out.println("Simulation run "+ (i+1) + " (" + (end - start) + "ms); Queue length at the end of simulation run is: " + temp);
			
				
			}
			average = (double) queueSizes / NUM_RUNS;
